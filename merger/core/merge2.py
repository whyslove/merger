...
class Merge:
    ...
    round_end_time = None
    cameras_file_name = None
    screens_file_name = None
    event_name = None
    ...
    def __init__(self, record: Record, room: Room):
        ...
        for cam_file_name, screen_file_name, reserve_cam_file_name in zip(cam_file_names, screen_file_names,
            ...
            reserve_cam_file_id = \
                self.screen_videos_check(cam_file_name, screen_file_name, reserve_cam_file_name, date_time_end)
    ...
    def create_merge(self) -> tuple:
        with LOCK:
            ...
            try:
                os.remove(f'{HOME}/vids/cam_clipped_{self.start_time}_{self.end_time}.mp4')
            except OSError:
                logger.exception(f"Error while deleting file cam_clipped_{self.start_time}_{self.end_time}.mp4")
